define(["require", "dojo/_base/lang", "dojo/_base/declare", "dojo/has", "dojo/on", "dojo/Deferred", "dojo/when",
	...
	return declare("dojox.app.controllers.Transition", Controller, {
		...
		transition: function(event){
			...
			//
			// web:
			//		Use emit to trigger "app-transition" event, and this function will response to the event. For web:
		...
		onStartTransition: function(evt){
			...
			//
			// web:
			//		Use "dojox/mobile/TransitionEvent" to trigger "startTransition" event, and this function will response the event. For web:
			//		|	var transOpts = {
			//		|		title:"List",
			//		|		target:"items,list",
			//		|		url: "#items,list",
		...
		_doTransition: function(transitionTo, opts, params, data, parent, removeView, doResize, forceTransitionNone, nested){
			...
			if(nextSubNames == currentSubNames && next == current){ // new test to see if current matches next
				...
				//view we want to start transition. For web, during transition 1 -> 2
