	...
	[
		...
		function testReadAPI_getValue(t){
			 ...
			 function onComplete(items, request){
				 ...
				 t.assertEqual(store.getValue(item,"id"), "http://web.com/samplefeedEdit.xml/entry/10");
				 d.callback(true);
			 }
			 function onError(error, request){
				 d.errback(error);
		...
		function testReadAPI_getValues(t){
			 ...
			 function onComplete(items, request){
				 ...
				 t.assertEqual(values[0], "http://web.com/samplefeedEdit.xml/entry/10");
				 d.callback(true);
			 }
			 function onError(error, request){
				 d.errback(error);
		...
		function testWriteAPI_setValues(t){
			...
			function onComplete(items, request){
				...
				var item = items[0];
				t.assertTrue(store.containsValue(item,"title", "Test Editable Entry #1"));
				store.setValues(item, "author", [{name: "John"}, {name: "Bill", email:"bill@web.com", uri:"http://web.com/bill"}]);
				var values = store.getValues(item,"author");
				t.assertEqual(values[0].name, "John");
				t.assertEqual(values[1].name, "Bill");
				t.assertEqual(values[1].email, "bill@web.com");
				t.assertEqual(values[1].uri, "http://web.com/bill");
				d.callback(true);
			}
			function onError(error, request){
				d.errback(error);
		...
		function testReadAPI_getIdentity(t){
			...
			function onComplete(items, request){
				...
				t.assertEqual("http://web.com/samplefeedEdit.xml/entry/10",id);
				d.callback(true);
			}
			function onError(error, request){
				d.errback(error);
		...
		function testReadAPI_fetchItemByIdentity(t){
			...
			store.fetchItemByIdentity({identity: "http://web.com/samplefeedEdit.xml/entry/10", onItem: onItem, onError: onError});
			return d; //Object
		},
		...
		function testReadAPI_fetchItemByIdentity_fails(t){
			...
			store.fetchItemByIdentity({identity: "http://web.com/samplefeedEdit.xml/entry/10/none", onItem: onItem, onError: onError});
			return d; //Object
		},
