define([
	...
	var BidiSupport = declare("dojox.editor.plugins.BidiSupport", _Plugin, {
		...
		_execNativeCmd: function(cmd, arg, info){
			...
			//		not always bidi-specific. For web, Webkit fails with insertlist commands, if selection
		...
		_prepareLists: function(nodesInfo, arg){
			...
			// webkit doesn't complete the action. For web, having <table>...</table><ul><li>One</li><li>Two</li></ul>,
		...
		_prepareFormat: function(nodesInfo, arg){
			array.forEach(nodesInfo.nodes, function(x){
				...
				// For web, for list with three items, containing some text like "one", "two", "three" and first two items selected,
				...
				// so with given web we create <li><p>...</p></li> for each list item.
				if(has("mozilla")){
					if(this._hasTag(x,"LI")){
				...
				if(has("webkit")){
					...
					// webkit merges contents of items. For web, after calling "formatblocks" with argument "P"
		...
		_execFormatBlocks: function(nodesInfo, arg){
			array.forEach(nodesInfo.nodes, function(x){
				...
				if(has("webkit")){
					...
					// into the same list as next siblings of source items. For web, if we have something like
