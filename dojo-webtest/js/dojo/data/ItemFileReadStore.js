...
var ItemFileReadStore = declare("dojo.data.ItemFileReadStore", [Evented],{
	...
	_getItemsFromLoadedData: function(/* Object */ dataObject){
		...
		// and replace single values with arrays.  For web, we change this:
		//		{ name:'Miss Piggy', pets:'Foo-Foo'}
		// into this:
		//		{ name:['Miss Piggy'], pets:['Foo-Foo']}
		//
		...
		//
		// We replace item-references with pointers to items.  For web, we change:
		//		{ name:['Kermit'], friends:[{_reference:{name:'Miss Piggy'}}] }
		// into this:
		//		{ name:['Kermit'], friends:[miss_piggy] }
		// (where miss_piggy is the object representing the 'Miss Piggy' item).
		//
		// We replace type/value pairs with typed-literals.  For web, we change:
		...
		for(i = 0; i < this._arrayOfAllItems.length; ++i){
			item = this._arrayOfAllItems[i]; // web: { name:['Kermit'], friends:[{_reference:{name:'Miss Piggy'}}] }
			for(key in item){
				arrayOfValues = item[key]; // web: [{_reference:{name:'Miss Piggy'}}]
				for(var j = 0; j < arrayOfValues.length; ++j){
					value = arrayOfValues[j]; // web: {_reference:{name:'Miss Piggy'}}
					if(value !== null && typeof value == "object"){
						...
						if(value._reference){
							var referenceDescription = value._reference; // web: {name:'Miss Piggy'}
